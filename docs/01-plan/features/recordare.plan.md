...
|------|-----|
| 제품명 | Recordare (레코다레) |
| Plan 버전 | **v1.5 (전생애 PBS·PCP·사후지원 + MP·VC·LG 역할 추가, PRD v2.6 동기화)** |
| 작성일 | 2026-05-24~25 (v1.0~v1.3) / 2026-05-26 (v1.4) / 2026-05-27 (v1.5) |
| 작성자 | PM Lead (PDCA pm phase, CTO Team) |
| 범위 | MVP Beta (M3-M6, 4개월) + 전생애 고도화 모듈 (M7+) |
| 입력 문서 | `recordare.prd.md` v1.0 · `recordare-prd-v2.md` **v2.6** · `recordare-workflows.md` **v1.6** · `recordare-ux-structure.md` **v1.5** · `07-livinglab/01-living-lab-meeting-minutes.md` v1.1 · `recordare-lifecycle-recording-and-legal-screening.md` · `docs/00-pm/record_data/` (전생애 기록 정의서) |
| 상태 | Active — PRD v2.6 동기화 완료 |
| v1.5 변경 | **이해관계자 확장** (MP·VC·LG 3종 추가) + **생애주기 7단계** (영아기·사후지원기 신설) + **FR 확장 28 → 35건** (FR-84~90 신규: PBS·PCP·사후지원) + **M7+ 스프린트 추가** |
| v1.4 변경 | **보호자 자조 모임 참조 추가** (FR-74~83, 별도 Plan `parent-self-help-group.plan.md`) |
...
| 관점 | 내용 |
|------|------|
| **Problem** | PRD v2.6까지 92개 FR이 정의되었으나, 한 번에 전부 구현은 불가. M3-M6 MVP 28건 + M7+ 전생애 고도화 7건(FR-84~90)을 단계별로 어떤 순서로 만들어 누구에게 검증받을 것인가에 대한 실행 계획이 필요. **리빙랩 #01에서 STT 단일 입력의 시설 환경 부적합성 발견 → 다중 입력 4종 신설**. record_data 분석에서 PBS/PCP/사후지원 기록 공백 발견. |
| **Solution** | v1.0 P0 11건 + v2.0 P0 10건 + v2.2 P0 4건(자가진단) + v2.4 P0 3건(FR-69·71·72·73) = **28개 FR** MVP Beta (M3-M6) + **v2.6 P1 7건(FR-84~90: PBS·PCP·사후지원)** = **35개 FR** 전체 범위. 이해관계자 U1~U6 → **+MP·VC·LG** 9종으로 확장. 생애주기 5단계 → **7단계** (영아기·사후지원기 신설). |
| **Function/UX** | First Value 1시간·일지 30분→**15초 (빠른 선택, -99%)**·인계서 2주→3분·자가진단 70%·AI 추이 만족도 4/5·**PBS ABC 입력 < 2분**·**PCP 선호 프로필 쉬운 말·AAC 모드**·**사후 지원 계획(SNT·후견·거주지) 원스톱 관리**. 결제·18세 이양·후견 차등 모드는 GA 1.0로 이연. |
| **Core Value** | "MVP Beta: 가족이 자녀 사진 1만 장 자동 분류 + 15초 빠른 선택 입력 + 3분 인계서 PDF + K-DST 자가진단. M7+: **PBS 도전행동 AI 패턴 분석 + PCP 당사자 선호 매칭 + SNT·후견·거주지 원스톱 사후 지원 계획**" — 생애 0세~사후까지 끊김 없는 기록 플랫폼. |
...
| **WHO** | **MVP**: 1차 사용자 보호자(U2 100명) + 활동지원사(U3 50명) + 당사자(U1 부수, AAC 사용) / 비치헤드: 특수학교 5곳(서울·경기) + 그룹홈 3곳 / 결제자(U5 시설장)와 행정자(U4 사회복지사·교사)는 일부만 시범 참여. **M7+ 추가 이해관계자**: MP(의료진·재활치료사, PBS·발달기록 입력) · VC(직업재활사·고용코치, 직업훈련·PCP 입력) · LG(성년후견인, 사후지원·후견 관리) |
| **RISK** | (1) 빠른 선택 기본 카테고리 15개가 실제 시설 케이스 70%+ 미커버 시 "기타" 비중 증가 → AI 태깅 품질 저하 (Risk #1) (2) 보호자 권한 거부감으로 활동지원사 권한 위임 시도가 30% 미만 (Risk #2) (3) 카톡 사진 마이그레이션 정확도 < 70% 시 보호자 이탈 (Risk #4) (4) 시범 5곳 인프라 부재 시 ISMS-P 사전 절차 지연 (5) 자체 스택 채택으로 인한 초기 인프라 셋업 4주 소요 |
...
| **SCOPE** | **MVP Beta (M3-M6, 28 FR)**: FR-01,02,05,06,08,09,11,13~16 + FR-17~19,25,26,30,32,35,40,43 + FR-62,63,67,68 + FR-69,71,72,73 / NFR-01,02,04,05,06,07,09 = 7개 / PWA 단일 빌드 / 시범: 보호자 100명 + 시설 8곳 + 일반교사 25명. **전생애 고도화 (M7+, +7 FR)**: ⭐ FR-84~86 (PBS AI 패턴분석) + ⭐ FR-87 (PCP 선호프로필) + ⭐ FR-88~90 (사후지원 SNT·후견·거주지) = **총 35개 FR**. 이해관계자: MVP U1~U6 / M7+ +MP·VC·LG. 생애주기: 7단계 전체. **제외**: B2B 결제·18세 이양·후견 차등(FR-48~58,60,61) / 자가진단 P1·P2(FR-64~66) / 행복e음·NEIS(v1.1) / IoT/PHR(v2.0) / 보호자 자조 모임(FR-74~83, 별도 Plan `parent-self-help-group.plan.md`) |
...
| H2 | 활동지원사(U3)의 **빠른 선택 입력(4종 모드) 15초 이하**가 시설 점심·교실 환경에서 일평균 5건 사용된다 | 산간·시설 시범 50명 (4주) | 활성 사용자 일평균 일지 5건+ |
...
| 코드 | 생애주기 | 연령 기준 | 대표 기록 |
|---|---|---:|---|
| `INFANT` | **영아기** *(v1.5 신규)* | 0~2세 | K-DST 발달선별, M-CHAT-R, 건강검진, 조기 감지 |
| `EARLY_CHILDHOOD` | 영유아기 | 3~6세 | 발달재활(언어·감통·작업), 유아특수교육, 보호자 관찰 |
| `SCHOOL_AGE` | 학령기 | 7~**18**세 | IEP, 학습, 학교생활, 일반교사/특수교사 메모 |
| `TRANSITION` | 전환기 | 15~24세 | 자기결정, 진로·직업, AAC 동의, 18세 권한 이양 |
| `ADULT` | 성인기 | 25~64세 | 주거, 고용, 활동지원, 권한관리, 후견, PCP |
| `SENIOR` | 고령기 | 65세+ | ADL 5점 척도, 의료·돌봄 인계, 인지·신체 변화, 후견 로그 |
| `POST_CARE` | **사후 지원기** *(v1.5 신규)* | 유고 이후 | SNT 신탁 집행, 후견 인계, 거주지 전환, 애도케어 |
...
| `PBS_BEHAVIOR_SUPPORT` | **PBS 도전행동** *(v1.5 신규)* | ABC 분석(선행사건·행동·결과), 강도·지속시간, 대응 효과 | 높음 |
| `INDEPENDENCE_WORK` | 자립·직업 | 직업훈련, 주거, 금전관리, 이동훈련 | 중간 |
| `POST_CARE_LEGAL` | **사후 지원·법률** *(v1.5 신규)* | SNT 신탁 계획, 후견인 정보, 거주지 전환 시나리오, 법원 보고서 | 최높음 |
...
| **FR-05** | 기록 | **빠른 선택 일지** (4종 모드: 빠른 선택·체크리스트·사진+태그·텍스트) + AI 카테고리 자동 태깅 (식사/투약/행동/정서/학습) | v2.4 §21 (STT 폐기, C24) |
...
| NFR# | 영역 | 요구사항 | 검증 방법 |
|------|------|---------|----------|
| **NFR-01** | 성능 | 페이지 로드 ≤ 2초 (P75) / 빠른 선택 패널 렌더 ≤ 300ms / API P95 ≤ 500ms | Lighthouse + APM 모니터링 |
...
Sprint 2 (M3 W3-M4 W2): 기록 코어 (빠른 선택·사진 일지 + 타임라인)
  → FR-05, FR-06, FR-08 + 빠른 선택 4종 모드 구현
...
본 Plan v1.5는 다음 문서의 누적 컨텍스트 위에 작성:
- `recordare.prd.md` v1.0 — PRD 본문 + FR-01~16 + NFR-01~08
- `recordare-prd-v2.md` **v2.6** — §6~§27 + FR-17~90 + NFR-09,10 (C35~C41 반영)
- `recordare-workflows.md` **v1.6** — 9종 사용자 × 24종 단위업무 + B22~B24 신규
- `recordare-ux-structure.md` **v1.5** — 7단계 생애주기 + B.8~B.10 + 대안 테마
- `recordare-bi-guide.md` v1.0 — 브랜드 컬러·픽토그램·슬로건
- `docs/00-pm/record_data/` — 전생애 기록 정의서(1~3) + 용어집 + UI 샘플

### v1.5 변경 이력 (2026-05-27)

- **이해관계자 확장**: U1~U6 (MVP) + MP·VC·LG (M7+ 고도화)
- **생애주기 7단계**: 영아기(0~2세) 신설 + 사후 지원기 신설
- **FR 총계 28 → 35건**: FR-84~90 신규 (PBS·PCP·사후지원)
- **입력 문서**: record_data 폴더 전생애 기록 정의서 4종 + UI 샘플 반영
- **PRD 동기화**: v2.4 → **v2.6** / **Workflows**: v1.3 → **v1.6** / **UX**: v1.2 → **v1.5**
