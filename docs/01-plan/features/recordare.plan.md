...
| 관점 | 내용 |
|------|------|
| **Problem** | PRD v2.2까지 68개 FR + 10개 NFR이 정의되었으나, 한 번에 전부 구현은 불가. M3-M6 4개월 안에 어떤 FR을 어떤 순서로 만들어 누구에게 검증받을 것인가에 대한 실행 계획이 필요. |
| **Solution** | v1.0 P0 11건(계정·권한·일지·타임라인·인계서·보안) + v2.0 P0 10건(당사자모드·온보딩·오프라인·알림·접근성) + **v2.2 P0 4건(K-DST·M-CHAT-R·AI 추이·면책)** = **25개 FR** MVP Beta. Next.js 15 + Node.js/Express + PostgreSQL + Redis 자체 스택, PWA 우선. 특수학교 5곳 + 신규 진단 보호자 100명에게 시범 운영. |
| **Function/UX** | First Value 1시간 도달율 60%+, 일지 작성 시간 30분→5분, 인계서 작성 2주→3분 달성. AAC 동의·쉬운말 모드·오프라인 동기화 모두 MVP 포함. **+ 신규 보호자 K-DST 자가진단 도달율 70%+ + AI 추이 분석 만족도 4/5 + 의료적 LLM 출력 0건**. 결제(B2B)·18세 이양·후견 모드는 GA 1.0 (M7-M12)로 이연. |
| **Core Value** | "4개월 안에 실제 가족이 자녀의 사진 1만 장을 자동 분류 받고, 신규 활동지원사에게 3분 인계서를 PDF로 전달하며, **신규 진단 보호자가 K-DST·M-CHAT-R로 발달 변화를 객관적으로 추적하는 흐름이 작동한다**" — 실제 임팩트를 4개월 안에 만든다. |
...
| **SCOPE** | **포함**: FR-01,02,05,06,08,09,11,13,14,15,16 + FR-17,18,19,25,26,30,32,35,40,43 + **FR-62,63,67,68 (자가진단 v2.2 P0)** = **25개** / NFR-01,02,04,05,06,07,09 = 7개 / PWA 단일 빌드 / Beachhead 시범 8곳 / **제외**: B2B 결제(FR-54~58 GA 1.0) / 18세 이양 가이드(FR-48~52 GA 1.0) / 후견 모드(FR-59~61 GA 1.0) / 케이스 회의 자료(FR-53 GA 1.0) / 자가진단 P1·P2(FR-64,65,66 GA/v1.1) / 행복e음·NEIS 연동(v1.1) / IoT/PHR(v2.0) |
